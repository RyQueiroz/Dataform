...
var query = (
  ...
  SELECT
    ...
    *
  FROM 
    ${ctx.ref("RVQ_ext_clientes2")}
  WHERE NOT (cliente_email like "%@%.%")
  `)

assertion.assertion('raw_RVQ_clientes_row_condition2',['RVQ_ext_clientes2'],query);
...
var schema = 'business';
var tags = ['business'];
var dependencia_managed = ['RVQ_tb_itens_pedidos_clientes2'];
...
query = (
  ...
  SELECT DISTINCT
      ...
      cliente_estado
  FROM 
      ${ctx.ref('RVQ_tb_itens_pedidos_clientes2')}
  WHERE
      cliente_cpf NOT IN (SELECT cliente_cpf
                          FROM ${ctx.ref('raw_RVQ_clientes_row_condition2')})  
  `);

transformation.transform(
  nome_da_tabela = 'RVQ_marketing_clientes2', 
  schema, 
  operacao = 'view', 
  tags, 
  dependencias = ['RVQ_tb_itens_pedidos_clientes2', 
        'raw_RVQ_clientes_row_condition2'], 
  query
);
...
transformation.transform(
  nome_da_tabela = 'RVQ_adm_receita_MG2', 
  schema, 
  operacao='view', 
  tags, 
  dependencia_managed, 
...
transformation.transform(
  nome_da_tabela = 'RVQ_adm_receita_SP2', 
  schema, 
  operacao='view', 
  tags, 
  dependencia_managed, 
...
transformation.transform(
  nome_da_tabela = 'RVQ_adm_receita_por_dia2', 
  schema, 
  operacao='view', 
  tags, 
  dependencia_managed, 
...
transformation.transform(
  nome_da_tabela = 'RVQ_adm_receita_por_estado2', 
  schema, 
  operacao='view', 
  tags, 
  dependencia_managed, 
