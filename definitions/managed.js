...
ctx =>`
    ...
    FROM 
        ${ctx.ref('RVQ_ext_clientes2')} cl
    JOIN 
        ${ctx.ref('RVQ_ext_enderecos2')} ad
        ON cl.cliente_cpf = ad.cliente_cpf
    JOIN 
        ${ctx.ref('RVQ_ext_itens_produtos_pedidos2')} it
        ON cl.cliente_cpf = it.pedido_cliente_cpf
    `)


transformation.transform(
    'RVQ_tb_itens_pedidos_clientes2', 
    'managed', 
    'table', 
    ['managed'], 
    ['RVQ_ext_clientes2', 'RVQ_ext_enderecos2', 
    'RVQ_ext_itens_produtos_pedidos2'], 
    query
)
